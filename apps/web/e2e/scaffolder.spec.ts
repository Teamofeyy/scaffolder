...
const recipeCatalog = [
  {
    ...
    description:
      'Vite, React, TypeScript, Tailwind CSS, and React Router baseline.',
    tier: 'recommended',
    status: 'active',
        ...
        values: [
          {
            id: 'none',
            label: 'None',
            description: 'Do not add a UI kit.',
            blocks: [],
          },
          {
            id: 'shadcn',
        ...
        values: [
          {
            id: 'none',
            label: 'None',
            description: 'Do not add tests.',
            blocks: [],
          },
          {
            id: 'vitest',
            label: 'Vitest',
            description: 'Add Vitest.',
            blocks: ['vitest'],
          },
        ],
      },
        ...
        values: [
          {
            id: 'none',
            label: 'None',
            description: 'Use React state only.',
            blocks: [],
          },
          {
            id: 'zustand',
            label: 'Zustand',
            description: 'Add Zustand.',
            blocks: ['zustand'],
          },
        ],
      },
...
test.beforeEach(async ({ page }) => {
  ...
  await page.route(
    '**/api/recipes/react-router-app/generate',
    async (route) => {
      await route.fulfill({
        status: 200,
        headers: {
...
test('switches locale through the header control', async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== 'chromium',
    'Locale switching is covered on desktop',
  )
  ...
  await expect(
    page.getByRole('heading', { name: 'Recipe workbench' }),
  ).toBeVisible()
  await expect(page.getByPlaceholder('Поиск рецептов')).toBeVisible()
})

test('opens a recipe, previews clickable files, and downloads a generated ZIP', async ({
  page,
}) => {
  await page.goto('/en')
...
test('keeps a mobile regression screenshot', async ({ page }, testInfo) => {
  test.skip(
    ...
    'Mobile screenshot runs only on the mobile project',
  )
