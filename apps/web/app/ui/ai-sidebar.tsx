...
export function AiSidebar({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await recommendProjectConfig(
        trimmed,
        sessionId(),
        config,
        locale,
      ...
      const description =
        err instanceof Error ? err.message : dictionary.recommendationError
                      ...
                      <div className="space-y-1 rounded-md border border-border/60 bg-card p-2">
                        {patchRows.map((row) => (
                          ...
                          >
                            ...
                            <span className="max-w-[58%] text-right font-medium">
                              {row.value}
                            </span>
                          </div>
                        ))}
...
function configPatchRows(
  ...
  if (patch.framework)
    rows.push({ label: labels.framework, value: patch.framework })
  ...
  if (patch.state_management)
    rows.push({ label: labels.state, value: patch.state_management })
  if (patch.dependencies?.length)
    rows.push({
      label: labels.dependencies,
      value: patch.dependencies.join(', '),
    })
  if (patch.dev_dependencies?.length)
    rows.push({
      label: labels.devDependencies,
      value: patch.dev_dependencies.join(', '),
    })
