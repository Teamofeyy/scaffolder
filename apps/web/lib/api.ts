...
export function validateRecipeProjectName(
  projectName: string,
  errors: ErrorDictionary,
) {
...
export async function getRecipe(recipeId: string): Promise<RecipeManifest> {
  try {
    const response = await axios.get(
      `${AGENT_URL}/recipes/${encodeURIComponent(recipeId)}`,
      {
        timeout: 10000,
      },
    )
...
export async function previewRecipe(
  ...
  try {
    ...
    throw new Error(
      recipeApiErrorMessage(error, 'Unable to build recipe preview'),
    )
  }
}
    ...
    if (axios.isAxiosError(error) && error.response?.data instanceof Blob) {
      ...
      if (apiError?.message) {
        const details = apiError.details
          ...
          : ''
        throw new Error(
          details ? `${apiError.message} ${details}` : apiError.message,
        )
      }
